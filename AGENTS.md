...
3. **UE5 参考**：如果需求涉及 UE5 已有功能，Agent 必须先读取 `UE5_REFERENCE_INDEX.md`，再制定 Unity 实现计划。
4. **代码修改后**：必须更新 `SYSTEM_INDEX.md`（新增/变更/删除项）。
5. **代码修改后**：必须更新 `DEVELOPMENT_LOG.md`（按日志模板追加记录）。
6. **逻辑不得堆放**：不允许把所有逻辑堆到 `PlayerController2D` 类中。
7. **卡牌数据**：所有卡牌数据必须使用 `ScriptableObject` 定义。
8. **系统拆分**：按以下子系统组织代码：
   - `Core` — 游戏入口、全局状态、事件总控
